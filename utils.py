...
class RAMACOTQueue(Queue):
    """A Cheapest-First-Out Queue."""
    ...
    def reorganize(self):
        ...
        for i in range(len(self.A)):
            ...
            for j in range(i, len(self.A)):
                if self.A[j].path_cost < min.path_cost:
                    min = self.A[j]
                    aux = j
                    found = 1
            if found == 1:
...
class RAMACOTHINTQueue(Queue):
    """A Closest-to-objective-First Queue."""
    ...
    def pop(self):
        ...
        for i in range(len(self.A)):
            min = self.A[i]
            for j in range(i, len(self.A)):
                if self.A[j].path_cost + self.problem.h(self.A[j]) < min.path_cost + self.problem.h(min):
                    min = self.A[j]
                    aux = j
                    found = 1
            if found == 1:
